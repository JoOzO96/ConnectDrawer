...
import com.example.jose.connectdrawer.Emitente.EmiteConfiguraService;
...
public class SincEmiteConfigura {
    public boolean iniciaAsinc(final Context context, final String ip) {
        ...
        EmiteConfiguraService emitenteService = retrofit.create(EmiteConfiguraService.class);
        final Call<EmiteConfigura> requestEmiteConfigura = emitenteService.emiteconfigura("1");
        final Response<EmiteConfigura>[] response = new Response[]{null};
        List<Field> listaCampos = new ArrayList<>(Arrays.asList(emiteConfigura.getClass().getDeclaredFields()));
        ...
        Thread thread = new Thread(
                new Runnable() {
                    ...
                    public void run() {
                        try {
                            response[0] = requestEmiteConfigura.execute();
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
        ...
        try {
            ...
            if (response[0].body() != null) {
                emiteConfigura = response[0].body();
            }
        ...
        if (emiteConfigura != null) {
            iniciaSinc(context, emiteConfigura);
            return true;
        } else {
            MostraToast mostraToast = new MostraToast();
            mostraToast.mostraToastLong(Sessao.retornaContext(),
    ...
    public void iniciaSinc(final Context context, final EmiteConfigura emiteConfigura) {
        ...
        final List<Field> fieldList = new ArrayList<>(Arrays.asList(emiteConfigura.getClass().getDeclaredFields()));
...
//        Cursor cursor = emitente.retornaEmitenteFiltradoCursor(context, emitente.getCodigoemitente());
        ...
        boolean status = emiteConfigura.cadastraEmiteConfigura(context, emiteConfigura);
